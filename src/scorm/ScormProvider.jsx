...
class ScormProvider extends Component {
    ...
    closeScormAPIConnection() {
        ...
        if (success) {
            ...
            if (this.props.debug)
                console.error(
                    "ScormProvider error: could not close the API connection"
                );
        }
    }
    ...
    setScore(scoreObj) {
        return new Promise((resolve, reject) => {
            ...
            if (typeof value === "number")
                promiseArr.push(
                    this.set(`cmi${coreStr}.score.raw`, value, true)
                );
            if (typeof min === "number")
                promiseArr.push(this.set(`cmi${coreStr}.score.min`, min, true));
            if (typeof max === "number")
                promiseArr.push(this.set(`cmi${coreStr}.score.max`, max, true));
            if (typeof status === "string")
                promiseArr.push(this.setStatus(status, true));
